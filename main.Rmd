...
```{r}
library(ROSE)
data_rose <-ROSE(target~. , data = train_transformed)$data

train_transformed
data_rose
table(train_transformed$target)
...
# weight_0 = nrow(train_folded) / (sum(train_folded$target == "0") *2)
# weight_1 = nrow(train_folded) / (sum(train_folded$target == "1") *2)
